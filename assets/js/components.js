...
function renderPublication(pub) {
  ...
  if (!imageSrc) {
    // No teaser available; render an empty placeholder so layout still aligns.
    imageHtml = '<div class="project-image-placeholder" aria-hidden="true"></div>';
  }
  // 1. 优先判断是否为 YouTube 链接
  else if (imageSrc.includes('www.youtube.com/embed/')) {
    imageHtml = `
  ...
  // 3. 最后默认为图片
    ...
    imageHtml = `<img class="img-fluid project-image rounded shadow-sm" loading="lazy" src="${imageSrc}" alt="${pub.title || 'teaser'}" />`;
  }
...
function initializePage() {
  ...
  if (newsList) {
    const visibleNews = newsData.slice(0, 8);
    const hiddenNews = newsData.slice(8);
